...
PostulateVerifierValues PostulateVerifier::B3(const Model& model) {
	...
	for (int p = 0; p < 3; ++p) {
		std::string pointVarName1 = model.m_PointDef.m_NumberedVarNames[0].toString() + (p == 1 ? 'b' : 'a');
		std::string pointVarName2 = model.m_PointDef.m_NumberedVarNames[0].toString() + (p == 2 ? 'b' : 'c');
		std::string isNotTheSameSmt = "(feqBiggerError " + pointVarName1 + "0 " + pointVarName2 + "0)";
		for (int i = 1; i < model.m_PointIdentifiers; ++i) {
			isNotTheSameSmt = "(and " + isNotTheSameSmt + " (feqBiggerError " + pointVarName1 + std::to_string(i) + " " + pointVarName2 + std::to_string(i) + "))";
		}
		smt += "(assert (not " + isNotTheSameSmt + "))";
	}
	...
	//  Standard functions (See top of document)
	//  Also feqBiggerError function for line comparison, to prevent rounding errors
...
ParallelType PostulateVerifier::PARALLEL(const Model& model) {
	...
	if (model.m_PointFromLines[0].m_EquationString == "") { return NORESULT; }
