...
TextInputFieldWithDesc::TextInputFieldWithDesc(float leftX, float rightX, float topY, float bottomY, const std::string& text, float width)
...
{
	m_SubUIElements.push_back({ std::make_shared<TextInputField>(leftX + width, rightX - 0.006f, topY, bottomY), true });
}
