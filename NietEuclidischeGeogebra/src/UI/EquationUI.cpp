	...
	: UIElement(leftX, rightX, topY, bottomY, "EquationUI")
{
	...
	m_SubUIElements.push_back({ std::make_shared<TextInputFieldWithDesc>(leftX, rightX, topY - 0.2f, topY - 0.4f, "Line definition: ", 0.2f), false });
	m_LineDefInputField = m_SubUIElements.size() - 1;
	m_SubUIElements.emplace_back(std::make_shared<KeyboardUI>(leftX, rightX, bottomY + 0.85f, bottomY));
	m_SubUIElements.emplace_back(std::make_shared<TabUI>(leftX, rightX, topY, topY - 0.2f, 0, &ButtonClickedCallback, this));
	m_SubUIElements.emplace_back(std::make_shared<ButtonUI>(leftX + 0.02f, rightX - 0.02f, bottomY + 1.15f, bottomY + 0.87f, UpdateGraphsStatic, this, "Update graphs"));
...
void EquationUI::UpdateGraphs()
{
	...
	{
		std::cerr << "Failed to create the line!\n";
	}
}
