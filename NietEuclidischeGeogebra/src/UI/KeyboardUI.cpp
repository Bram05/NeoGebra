...
{    
	...
	for (int i=0; i < 10; i++) {
		...
		m_SubUIElements.push_back({std::make_shared<ButtonUI>(leftX + indent + x * buttonWidth, (leftX + x * buttonWidth + buttonWidth), topY - 0.01-0.1f, (topY - 0.09f-0.1f), functions[i], this, textList[i])});
		x++;		
	}
}    
